  ...
  async loadData(): Promise<StuntingData[]> {
    try {
      ...
      if (error.name === "AbortError") {
        throw new Error(
          "Koneksi ke backend timeout. Pastikan server backend berjalan di http://localhost:3001"
        );
      }

      if (error.message.includes("fetch")) {
        throw new Error(
          "Tidak dapat terhubung ke backend server. Pastikan server backend berjalan di http://localhost:3001"
        );
      }
