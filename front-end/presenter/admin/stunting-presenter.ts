...
export class StuntingPresenter {
  ...
  async getDetailedRecordResult(id: string) {
    try {
      this.view.setUIState({ isLoading: true });
      const detailedResult = await this.model.getDetailedRecordResult(id);
      this.view.setUIState({ isLoading: false });
      return detailedResult;
    } catch (error) {
      this.view.setUIState({
        errorMessage: "Gagal memuat detail hasil pemeriksaan",
        isLoading: false,
      });
      return null;
    }
