...
import {
  EdukasiPresenter,
  type EdukasiView,
} from "../presenter/admin/edukasi-presenter";
import {
  type Edukasi,
...
export default function KelolaEdukasi() {
  ...
  const presenter = new EdukasiPresenter({
    ...
    displayLoading: (loading) => setIsLoading(loading),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await presenter.saveEdukasi(formData, editingEdukasi?.id || null);
  };
  ...
  const addContentSection = () => {
    setFormData((prev) => ({
      ...prev,
      content: presenter.addContentSection(prev.content),
    }));
  };
  ...
  const addIllustrationToContent = (id: string) => {
    setFormData((prev) => ({
      ...prev,
      content: presenter.addIllustrationToContent(prev.content, id),
    }));
  };
  ...
  const addImportantPoint = () => {
    setFormData((prev) => ({
      ...prev,
      importantPoints: presenter.addImportantPoint(prev.importantPoints),
    }));
  };
  ...
  const removeImportantPoint = (index: number) => {
    setFormData((prev) => ({
      ...prev,
      importantPoints: presenter.removeImportantPoint(
        prev.importantPoints,
        index
      ),
    }));
  };
  ...
  const updateImportantPoint = (index: number, value: string) => {
    setFormData((prev) => ({
      ...prev,
      importantPoints: presenter.updateImportantPoint(
        prev.importantPoints,
        index,
        value
      ),
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Apakah Anda yakin ingin menghapus Edukasi ini?")) {
      await presenter.deleteEdukasi(id);
    }
  };
  ...
  const generateTableOfContents = () => {
    return formData.content
      ...
      .concat(
        formData.conclusion.paragraph.trim() !== ""
          ? [formData.conclusion.h2]
          : []
      );
  };
