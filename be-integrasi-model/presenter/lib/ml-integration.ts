...
const BACKEND_HOSTS = ["http://localhost:3001", "http://127.0.0.1:3001", "http://192.168.56.1:3001"]
 ...
 */
export function validateInputRanges(input: PredictionInput): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
 ...
 */
async function tryMultipleHosts<T>(operation: (client: any) => Promise<T>): Promise<T> {
  const hosts = [BACKEND_URL, ...BACKEND_HOSTS.filter((h) => h !== BACKEND_URL)]
 ...
 */
export async function predictStunting(input: PredictionInput): Promise<PredictionResult | PredictionError> {
  try {
